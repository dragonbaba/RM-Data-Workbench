...
const GAME_EFFECT_TYPE_DEFINITIONS: GameEffectTypeDefinition[] = [
  ...
  createTypeDefinition({
    ...
    argsMode: 'ops',
    exampleName: 'C 装迎击补正',
    exampleDescription: '每个装备中的 C 装置会独立应用自身携带的属性奖励，可随多个 C 装叠加',
    selectorTemplate: {},
  ...
  createTypeDefinition({
    ...
    argsMode: 'ops',
    exampleName: '底盘会心补正',
    exampleDescription: '每个装备中的底盘会独立应用自身携带的属性奖励，可随多个底盘来源叠加',
    selectorTemplate: {},
    argsTemplate: { ops: [{ group: 'extraParams', key: 'critRate', op: 'add', value: 15 }] },
    allowedGroups: OWNER_STATIC_GROUPS,
  }),
  createTypeDefinition({
  ...
  createTypeDefinition({
    effectType: 'base_slot_action_repeat_bonus',
    label: '底盘槽位追加发射',
    isStatic: true,
    selectorMode: 'equip',
    argsMode: 'ops',
    exampleName: '底盘火力全开',
    exampleDescription: '底盘给指定槽位武器追加发射次数',
    selectorTemplate: { ...FULL_EQUIP_SELECTOR_TEMPLATE, etypeIds: [10, 11, 12] },
    argsTemplate: { ops: [{ group: 'vehicleParams', key: 'actionRepeat', op: 'add', value: 1 }] },
    allowedGroups: ACTION_REPEAT_GROUPS,
  }),
  createTypeDefinition({
